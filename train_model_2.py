...
def tf_loss_fn_us(y_true, y_pred):
    ...
    D = tf.stack(D)
    epsilon = 0.000001
    Ed1 = tf.math.reduce_sum((tf.math.log((x_trains_old[0][:,0]+epsilon))-tf.math.log((I+epsilon)))**2 + (tf.math.log((x_trains_old[0][:,1]+epsilon))-tf.math.log((D+epsilon)))**2)

    Ed2 = 0.01*tf.math.log(tf.math.reduce_max(x_trains_old[0][:,0])+epsilon)/(tf.math.reduce_max(x_trains_old[0][:,0])+epsilon) * tf.math.reduce_sum((x_trains_old[0][:,0]-I)**2+ (x_trains_old[0][:,1]-D)**2)

    sumr = 0
    for it in range(num_times-1):
        sumr = sumr + (beta_t[it]-beta_t[it+1])**2 + (gamma_t[it]-gamma_t[it+1])**2 + 100*(mu_t[it]-mu_t[it+1])**2
    Er = 100*tf.math.log(tf.math.reduce_max(x_trains_old[0][:,0])+epsilon)/tf.math.reduce_max([beta_0,gamma_0,mu_0])*sumr

    E0 = 100*tf.math.log(tf.math.reduce_max(x_trains_old[0][:,0])+epsilon)/tf.math.reduce_max([beta_0,gamma_0,mu_0])*((beta_t[0]-beta_0)**2+(gamma_t[0]- gamma_0)**2 +100*(mu_t[0]-mu_0)**2)
...
#layer = MyDenseLayer(num_times*3)
#_ = layer(tf.zeros([num_times,1])) # Calling the layer `.builds` it.
...
model = tf.keras.models.Sequential([
    ...
    MyDenseLayer(num_times*3),
    tf.keras.layers.Activation('sigmoid'),
    tf.keras.layers.Reshape((num_times, 3))
])

model.compile(optimizer=tf.keras.optimizers.Adam(0.006),
                loss = 'mean_squared_error',
                metrics=['accuracy']
                )

model.fit(x_trains,y_trues,verbose=0,epochs=5000)
...
multiplier = 1/10
for t in range(1,20):
    optimizer = tf.keras.optimizers.Adam(learning_rate = 0.5*multiplier)
